// ============================================
// GAME LOGIC - WITH MAP SYSTEM INTEGRATION
// ============================================
...
let waveStartTime = 0;
let currentMap = 'ARENA'; // Default map
...
// Obstacles array (populated by map system)
let obstacles = [];
...
// Function to load a map (will be called from obstacles.js)
function loadMap(mapId) {
    if (typeof window.MAP_PRESETS !== 'undefined' && window.MAP_PRESETS[mapId]) {
        currentMap = mapId;
        const mapData = window.MAP_PRESETS[mapId];
        
        // Apply map background color
        document.body.style.backgroundColor = mapData.backgroundColor;
        
        // Regenerate obstacles for current wave
        if (typeof window.generateObstaclesForWave === 'function') {
            window.generateObstaclesForWave(wave);
            obstacles = window.obstacles || [];
        }
        
        queueMessage(`Map changed to: ${mapData.name}`);
        return true;
    }
    return false;
}
...
function getCurrentMap() {
    return window.MAP_PRESETS ? window.MAP_PRESETS[currentMap] : null;
}

// Draw ground texture based on current map
function drawGroundTexture() {
    if (typeof window.drawGroundTexture === 'function' && getCurrentMap()) {
        const mapData = getCurrentMap();
        window.drawGroundTexture(ctx, 0, 0, canvas.width, canvas.height, mapData.groundTexture, Date.now());
    } else {
        // Fallback grid
...
function findPathToTarget(enemy, target) {
    ...
    for (let obs of obstacles) {
        if (obs.type && obs.type.solid) {
            const obsDx = enemy.x - obs.x;
            const obsDy = enemy.y - obs.y;
            const obsDist = Math.sqrt(obsDx * obsDx + obsDy * obsDy);
            const minDist = enemy.radius + Math.max(obs.width, obs.height) / 2;
            
            if (obsDist < minDist + 50) {
                const avoidForce = (minDist + 50 - obsDist) / obsDist;
                moveX += obsDx * avoidForce * 0.3;
                moveY += obsDy * avoidForce * 0.3;
            }
        }
    }
    
    // Avoid healing towers
...
function createStatsPanel() {
    ...
    panel.innerHTML = `
        ...
        <div class="stats-content">
            ...
            <div class="stat-row"><span class="stat-label">🗺️ Map:</span><span class="stat-value" id="stat-map">Arena</span></div>
...
function updateStatsPanel() {
    ...
    document.getElementById('stat-map').textContent = mapData ? mapData.name : 'Arena';
...
function saveGame() {
    ...
    const gameData = {
        wave, gold, kills, gameState, waveActive, refreshCount, refreshCost, currentMap,
        player: {
...
function loadGame() {
    ...
    try {
        ...
        if (gameData.currentMap) currentMap = gameData.currentMap;
...
function startWave() {
    ...
    monsters = []; player.projectiles = []; player.meleeAttacks = []; visualEffects = [];
    bossProjectiles = []; monsterProjectiles = []; dashers = []; splitterTracking = [];
    scrapWeaponBtn.style.display = 'none'; mergeWeaponBtn.style.display = 'none';
    selectedWeaponIndex = -1; mergeTargetIndex = -1;
    
    // Generate obstacles for the wave using current map
    if (typeof window.generateObstaclesForWave === 'function') {
        window.generateObstaclesForWave(wave);
        obstacles = window.obstacles || [];
    }
...
function endWave() {
    ...
    if (asteroidTimer) { clearInterval(asteroidTimer); asteroidTimer = null; }
    if (minionSpawnInterval) { clearInterval(minionSpawnInterval); minionSpawnInterval = null; }
    bossAbilities.asteroids = []; bossAbilities.slowField = null; bossAbilities.enraged = false;
    bossAbilities.bossWeapon = null; bossAbilities.bossWeaponAttack = 0; bossAbilities.bossDash = false;
    const waveConfig = getWaveConfig(wave);
...
function initGame() {
    ...
    Object.assign(player, {
        ...
        projectiles: [], meleeAttacks: [], ammoPack: false, dodgeChance: 0, thornsDamage: 0,
        attackSpeedMultiplier: 1, firstHitReduction: false, firstHitActive: false, voidCrystalChance: 0,
        guardianAngelUsed: false, consumables: [], berserkerRing: false, sharpeningStone: false,
        sharpeningStoneWave: 0, enchantersInk: false, guardianAngel: false, bloodContract: false,
        bloodContractStacks: 0, bloodContractInterval: null, lastBloodDamage: 0, inSlowField: false,
        slowFieldTicks: 0, lastSlowFieldTick: 0, facingAngle: 0, lastFacingAngle: 0
    });
    
    attackedMonsters.clear(); weaponTargets.clear();
    playerTowers.landmines.count = 0; playerTowers.landmines.active = [];
    playerTowers.healingTowers.active = []; placedBombs = [];
    
    const handgun = getWeaponById('handgun');
    player.weapons.push(new WeaponInstance(handgun));
    
    wave = 1; gold = GAME_DATA.PLAYER_START.gold; kills = 0;
    gameState = 'wave'; waveActive = false;
    selectedWeaponIndex = -1; mergeTargetIndex = -1; visualEffects = [];
    refreshCount = 0; refreshCost = 5;
    refreshCostSpan.textContent = '5g'; refreshCounter.textContent = 'Refreshes: 0';
    
    groundFire = []; poisonClouds = []; voidZones = []; activeTraps = [];
    bossProjectiles = []; monsterProjectiles = []; dashers = []; splitterTracking = [];
...
function gameLoop() {
    ...
    // Draw obstacles from map system
    if (typeof window.drawObstacles === 'function') {
...
function updateGame(deltaTime) {
    ...
    if (bossAbilities.bossWeapon && wave !== 30) {
        const boss = monsters.find(m => m.isBoss);
        if (boss && Math.sqrt(Math.pow(player.x - boss.x, 2) + Math.pow(player.y - boss.y, 2)) <= bossAbilities.bossWeapon.range &&
...
function updateWeapons(currentTime) {
    weaponTargets.clear();
