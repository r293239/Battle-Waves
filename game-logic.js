...
class WeaponInstance {
        ...
        if (this.type === 'ranged') {
            if (this.id === 'shotgun') {
                ...
                for (let i = 0; i < this.pelletCount; i++) {
                    ...
                    attacks.push({
                        type: 'ranged', x: playerX, y: playerY, angle: angle, speed: this.projectileSpeed,
                        range: this.range, damage: this.baseDamage, color: this.projectileColor,
                        weaponId: this.id, animation: this.animation, isPellet: true,
                        startTime: currentTime, size: 3, weaponRef: this
                    });
                }
                return attacks;
                ...
                return {
                    type: 'ranged', x: playerX, y: playerY, startX: playerX, startY: playerY,
                    angle: angle, speed: this.projectileSpeed, returnSpeed: this.returnSpeed,
                    range: this.range, damage: this.baseDamage, color: this.projectileColor,
                    weaponId: this.id, animation: this.animation, isBoomerang: true,
                ...
                return {
                    type: 'ranged', x: playerX, y: playerY, angle: finalAngle, speed: this.projectileSpeed,
                    range: this.range, damage: this.baseDamage, color: this.projectileColor,
                    weaponId: this.id, animation: 'knife', isThrowable: true, startTime: currentTime,
                    size: this.projectileSize || 6, spinSpeed: this.spinSpeed || 0, rotation: 0, weaponRef: this
                };
                ...
                return {
                    type: 'ranged', x: playerX, y: playerY, angle: angle, speed: this.projectileSpeed,
                    range: this.range, damage: this.baseDamage, color: this.projectileColor,
                    weaponId: this.id, animation: 'sniper', startTime: currentTime, size: 6,
                    weaponRef: this, sniper: true
                };
                ...
                return {
                    type: 'ranged', x: playerX, y: playerY, angle: angle, speed: this.projectileSpeed,
                    range: this.range, damage: this.baseDamage, color: this.projectileColor,
                    weaponId: this.id, animation: 'bolt', startTime: currentTime, size: 5,
                    weaponRef: this, pierceCount: this.pierceCount, piercedEnemies: []
                };
            } else {
                ...
                return {
                    type: 'ranged', x: playerX, y: playerY, angle: angle, speed: this.projectileSpeed,
                    range: this.range, damage: this.baseDamage, color: this.projectileColor,
                    weaponId: this.id, animation: this.animation, bounceCount: this.bounceCount,
                    bounceRange: this.bounceRange, targetsHit: [], startTime: currentTime, size: 4, weaponRef: this
                };
            }
        } else {
            const angle = Math.atan2(targetY - playerY, targetX - playerX);
            if (this.dualStrike) {
                return {
                    ...
                    color: this.swingColor, startTime: Date.now(), duration: 200, swingAngle: this.swingAngle,
                    meleeType: 'dual', angle: angle, pierceCount: this.pierceCount, weaponId: this.id,
                    animation: this.animation, trailColor: this.trailColor, sparkleColor: this.sparkleColor,
                    bladeColor: this.bladeColor, hiltColor: this.hiltColor, gripColor: this.gripColor, dualStrike: true
                };
            }
            return {
                type: 'melee', x: playerX, y: playerY, radius: this.range, damage: this.baseDamage,
                color: this.swingColor, startTime: Date.now(), duration: 300, swingAngle: this.swingAngle,
                meleeType: this.meleeType, angle: angle, pierceCount: this.pierceCount, weaponId: this.id,
                animation: this.animation, trailColor: this.trailColor, sparkleColor: this.sparkleColor,
                shockwaveColor: this.shockwaveColor, shockwaveIntensity: this.shockwaveIntensity, tier: this.tier,
                bladeColor: this.bladeColor, hiltColor: this.hiltColor, handleColor: this.handleColor,
                headColor: this.headColor, edgeColor: this.edgeColor, shaftColor: this.shaftColor,
                prongColor: this.prongColor, tipColor: this.tipColor, gripColor: this.gripColor
            };
        }
    }
    ...
    getShopCost() {
        ...
        else if (this.tier > 2) return Math.floor(this.baseCost * Math.pow(1.8, this.tier - 1));
...
function getTargetPriority(monster, player, weapon, currentTime) {
    ...
    if (weapon.sniper) {
        ...
        monsters.forEach(other => {
            ...
            const odist = Math.hypot(other.x - monster.x, other.y - monster.y);
...
function spawnRandomLandmine() {
    ...
    while (!validPosition && attempts < 100) {
        ...
        if (distanceFromPlayer < minDistanceFromPlayer) { attempts++; continue; }
        
        let tooCloseToOtherMine = false;
        for (let mine of playerTowers.landmines.active) {
            if (Math.hypot(x - mine.x, y - mine.y) < minDistanceFromOtherMines + mine.radius) {
                tooCloseToOtherMine = true;
                break;
            }
        }
        if (!tooCloseToOtherMine) validPosition = true;
        attempts++;
    }
    ...
    playerTowers.landmines.active.push({
        x, y, radius: 15, damage: 80, explosionRadius: 60, active: true,
        startTime: Date.now(), color: '#8B4513', triggerColor: '#FF0000'
    });
...
function checkLandmineTriggers() {
    for (let i = playerTowers.landmines.active.length - 1; i >= 0; i--) {
        ...
        for (let j = 0; j < monsters.length; j++) {
            ...
            if (Math.hypot(monster.x - mine.x, monster.y - mine.y) < mine.radius + monster.radius) {
                for (let k = monsters.length - 1; k >= 0; k--) {
                    const m = monsters[k];
                    if (Math.hypot(m.x - mine.x, m.y - mine.y) < mine.explosionRadius + m.radius) {
                        m.health -= mine.damage;
                        createDamageIndicator(m.x, m.y, mine.damage, true);
                        if (m.health <= 0) handleMonsterDeath(m, k);
                    }
                }
...
function placeHealingTower() {
    ...
    playerTowers.healingTowers.active.push({
        x: player.x, y: player.y, radius: 20, health: 30, healAmount: 1,
        lastHeal: Date.now(), id: Date.now() + Math.random()
    });
...
function detonateBomb(bomb) {
    ...
    for (let i = monsters.length - 1; i >= 0; i--) {
        ...
        if (Math.hypot(monster.x - bomb.x, monster.y - bomb.y) < bomb.explosionRadius + monster.radius) {
...
function useExpScroll() {
    ...
    queueMessage(`${weapon.name} upgraded from Tier ${oldTier} to Tier ${weapon.tiger}!`);
    updateWeaponDisplay();
}
...
function getWaveConfig(waveNumber) {
    if (waveNumber <= GAME_DATA.WAVES.length) {
        ...
        return {
            number: waveData.number, monsters: waveData.monsters, monsterHealth: waveData.monsterHealth,
            monsterDamage: waveData.monsterDamage, goldReward: waveData.goldReward,
            isBoss: waveData.isBoss || false, minions: waveData.minions || 0, spawnDelay: waveData.spawnDelay || 300
        };
    } else {
        ...
        return {
            number: waveNumber, monsters: Math.floor(baseWave.monsters * scaleFactor),
            ...
            isBoss: (waveNumber % 10 === 0), minions: (waveNumber % 10 === 0) ? 8 : 0,
...
const WAVE_COMPOSITIONS = {
    ...
    10: { normal: 0, fast: 0, tank: 0, explosive: 0, gunner: 0, splitter: 0, dasher: 0, vampire: 0 },
    ...
    20: { normal: 0, fast: 0, tank: 0, explosive: 0, gunner: 0, splitter: 0, dasher: 0, vampire: 0 },
    ...
    30: { normal: 0, fast: 0, tank: 0, explosive: 0, gunner: 0, splitter: 0, dasher: 0, vampire: 0 },
...
function generateShopItems() {
    ...
    let availableWeapons = GAME_DATA.WEAPONS.filter(w => {
        ...
        return !player.weapons.some(pw => pw.id === w.id && pw.tier >= 5);
    });
    for (let i = 0; i < 2; i++) {
    ...
    let availableItems = [...GAME_DATA.ITEMS].filter(item => !(item.id === 'landmine' && playerTowers.landmines.count >= playerTowers.landmines.max));
    for (let i = 0; i < 2; i++) {
        if (availableItems.length > 0) {
            const randomIndex = Math.floor(Math.random() * availableItems.length);
            const item = { ...availableItems[randomIndex] };
            shopItems.push({ type: item.type === 'tower' ? 'tower' : 'item', data: item });
...
function showSpawnIndicators() {
    ...
    for (let i = 0; i < totalMonsters; i++) {
        ...
        spawnIndicators.push({ x, y, timer: 2000, startTime: Date.now(), isBoss: waveConfig.isBoss && i === 0, isMinion: false, index: i });
    }
}
...
function startWave() {
    ...
    if (!waveConfig.isBoss) {
        ...
        if (boss) {
            ...
            const monsterCount = waveConfig.monsters;
            const monsterTypes = getMonsterTypeForWave(wave);
            const baseSpawnDelay = waveConfig.spawnDelay || 500;
            let spawnedCount = 0;
            const spawnInterval = setInterval(() => {
                if (gameState !== 'wave') { clearInterval(spawnInterval); return; }
                if (spawnedCount >= monsterCount) { clearInterval(spawnInterval); return; }
...
function spawnAsteroid() {
    ...
    setTimeout(() => {
        ...
        for (let i = monsters.length - 1; i >= 0; i--) {
            const monster = monsters[i];
            if (Math.hypot(monster.x - x, monster.y - y) < radius + monster.radius && !monster.isBoss) {
                monster.health -= 75;
...
function handleMonsterDeath(monster, index) {
    ...
    if (monster.explosive) {
        ...
        monsters.forEach((otherMonster, otherIndex) => {
            ...
            if (Math.hypot(otherMonster.x - monster.x, otherMonster.y - monster.y) < explosionRadius + otherMonster.radius) {
...
function drawPlayer() {
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    if (isMoving) { ctx.moveTo(12, -2); ctx.lineTo(18, 0); ctx.lineTo(12, 2); }
    else { ctx.moveTo(12, -1); ctx.lineTo(18, 0); ctx.moveTo(12, 1); ctx.lineTo(18, 0); }
    ctx.stroke();
    ctx.restore();
    ctx.save();
    ctx.rotate(facingAngle);
...
function updateGame(deltaTime) {
    ...
    if (bossAbilities.slowField && bossAbilities.slowField.active) {
        const boss = monsters.find(m => m.isBoss && wave === 30);
        if (boss) {
            const distToField = Math.hypot(player.x - boss.x, player.y - boss.y);
    ...
    if (bossAbilities.bossWeapon && wave !== 30) {
        ...
        if (boss && Math.hypot(player.x - boss.x, player.y - boss.y) <= bossAbilities.bossWeapon.range && currentTime - (bossAbilities.bossWeapon.lastAttack || 0) > 2000) {
            ...
            bossAbilities.bossWeaponAttack = {
                ...
                meleeType: bossAbilities.bossWeapon.meleeType, angle: angle, pierceCount: bossAbilities.bossWeapon.pierceCount || 1
            };
...
function shootGunnerProjectile(gunner) {
    ...
    monsterProjectiles.push({
        ...
        radius: 5, color: MONSTER_TYPES.GUNNER.projectileColor, startTime: currentTime, lifetime: 3000
    });
}
...
function shootBossProjectiles(boss) {
    ...
    if (wave === 10) {
        ...
        for (let i = -2; i <= 2; i++) {
            ...
            bossProjectiles.push({ x: boss.x, y: boss.y, vx: Math.cos(angle) * 6, vy: Math.sin(angle) * 6, damage: 10, radius: 5, color: '#ff8888', startTime: currentTime, lifetime: 2500, isHoming: false });
        }
        bossProjectiles.push({ x: boss.x, y: boss.y, vx: Math.cos(baseAngle) * 5, vy: Math.sin(baseAngle) * 5, damage: 12, radius: 6, color: '#ff4444', startTime: currentTime, lifetime: 4000, isHoming: true, homingStrength: 0.03, target: { x: player.x, y: player.y } });
    } else if (wave === 20) {
        // 4 wide-spread + 2 homing
        for (let i = -2; i <= 2; i++) {
            if (i === 0) continue;
            const angle = baseAngle + (i * 0.4);
            bossProjectiles.push({ x: boss.x, y: boss.y, vx: Math.cos(angle) * 5, vy: Math.sin(angle) * 5, damage: 12, radius: 6, color: '#ff4444', startTime: currentTime, lifetime: 3000, isHoming: false });
        }
        for (let h = -1; h <= 1; h++) {
            if (h === 0) continue;
            const angle = baseAngle + (h * 0.2);
            bossProjectiles.push({ x: boss.x, y: boss.y, vx: Math.cos(angle) * 4.5, vy: Math.sin(angle) * 4.5, damage: 15, radius: 7, color: '#ff6600', startTime: currentTime, lifetime: 4000, isHoming: true, homingStrength: 0.04, target: { x: player.x, y: player.y } });
        }
    } else if (wave === 30) {
        ...
        for (let i = -3; i <= 3; i++) {
            ...
            bossProjectiles.push({ x: boss.x, y: boss.y, vx: Math.cos(angle) * 5.5, vy: Math.sin(angle) * 5.5, damage: 14, radius: 6, color: '#ff4444', startTime: currentTime, lifetime: 3000, isHoming: false });
        }
        for (let h = -1; h <= 1; h++) {
            const angle = baseAngle + (h * 0.25);
            bossProjectiles.push({ x: boss.x, y: boss.y, vx: Math.cos(angle) * 5, vy: Math.sin(angle) * 5, damage: 18, radius: 8, color: '#ff0000', startTime: currentTime, lifetime: 4500, isHoming: true, homingStrength: 0.05, target: { x: player.x, y: player.y } });
        }
    } else {
        for (let i = -2; i <= 2; i++) {
            const angle = baseAngle + (i * 0.25);
            bossProjectiles.push({ x: boss.x, y: boss.y, vx: Math.cos(angle) * 5, vy: Math.sin(angle) * 5, damage: 15, radius: 8, color: '#ff4444', startTime: currentTime, lifetime: 3000, isHoming: false });
        }
    }
}
    ...
    for (let i = player.projectiles.length - 1; i >= 0; i--) {
        ...
        if (projectile.isBoomerang) {
            if (projectile.state === 'outgoing') {
                ...
                if (Math.hypot(dx, dy) < 10) { player.projectiles.splice(i, 1); continue; }
            ...
            projectile.x += Math.cos(projectile.angle) * projectile.speed;
            projectile.y += Math.sin(projectile.angle) * projectile.speed;
            if (Math.hypot(projectile.x - player.x, projectile.y - player.y) > projectile.range) { player.projectiles.splice(i, 1); continue; }
        }
    ...
    for (let i = player.meleeAttacks.length - 1; i >= 0; i--) {
        ...
        for (let j = monsters.length - 1; j >= 0; j--) {
            ...
            if (Math.hypot(monster.x - attack.x, monster.y - attack.y) < attack.radius + monster.radius) {
                ...
                if (monster.health <= 0) { handleMonsterDeath(monster, j); j--; }
            }
        }
    }
}
        ...
        if (distToTarget < monster.radius + (targetIsTower ? 20 : player.radius) && currentTime - monster.lastAttack >= monster.attackCooldown) {
            if (targetIsTower && targetTower) {
                ...
                if (targetTower.health <= 0) {
                    ...
                    if (index > -1) { playerTowers.healingTowers.active.splice(index, 1); addVisualEffect({ type: 'explosion', x: targetTower.x, y: targetTower.y, radius: 30, color: '#8B0000', startTime: currentTime, duration: 300 }); queueMessage("Healing Tower destroyed!"); }
                }
            } else {
    ...
    visualEffects.forEach(effect => {
        ...
        switch(effect.type) {
            ...
            case 'spawn':
                ...
                ctx.shadowBlur = 15 * alpha;
                for (let i = 0; i < 3; i++) { ctx.beginPath(); ctx.arc(effect.x, effect.y, 15 + i * 10 + progress * 30, 0, Math.PI * 2); ctx.stroke(); }
                break;
            case 'bossSpawn':
...
function drawMonsters() {
    ...
    monsters.forEach(monster => {
        ...
        if (monster.isVampire) { ctx.strokeStyle = '#FF0000'; ctx.lineWidth = 2; ctx.shadowColor = '#FF0000'; ctx.shadowBlur = 10; ctx.beginPath(); ctx.arc(0, 0, monster.radius + 3, 0, Math.PI * 2); ctx.stroke(); }
        ctx.shadowBlur = 0;
        ctx.strokeStyle = '#000000';
        ctx.lineWidth = 2;
        ctx.beginPath();
...
document.addEventListener('keydown', (e) => {
    ...
    if (key === 'r') { if (gameState === 'shop') { player.weapons.forEach(weapon => { if (weapon.usesAmmo && !weapon.isReloading && !weapon.isThrowable) weapon.startReload(); }); } e.preventDefault(); }
